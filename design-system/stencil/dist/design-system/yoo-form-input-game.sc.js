...
class CardGame extends AbstractGame {
    ...
    initDeck() {
        ...
        let positions = [
            { x: 10, y: 15 },
            { x: this.gameInstance.world.width - (this.gameOptions.cardWidth * this.gameOptions.cardScale) - 5, y: 15 },
            { x: 10, y: this.gameInstance.world.height - (this.gameOptions.cardHeight * this.gameOptions.cardScale) - 15 },
            { x: this.gameInstance.world.width - (this.gameOptions.cardWidth * this.gameOptions.cardScale) - 5, y: this.gameInstance.world.height - (this.gameOptions.cardHeight * this.gameOptions.cardScale) - 15 }
        ];
        shuffle(positions);
        ...
        setTimeout(() => { this.hideAllCards(); this.mainLabel.setText(''); }, this.gameOptions.initViewDuration);
    }
    initHUD() {
        ...
        this.mainLabel = this.gameInstance.add.text(0, 0, 'MEMORY-GAME-INSTRUCTIONS', { fontSize: fontSize + 'px', fontWeight: 'bold', fill: '#fff', boundsAlignH: 'center', boundsAlignV: 'middle', backgroundColor: this.gameOptions.mainLabelColor });
    ...
    createCard(cardIndex, pos, initRevealed) {
        ...
        card.data = { idx: cardIndex, text: this.values[cardIndex], revealed: initRevealed === true, solved: false };
    ...
    createCardText(sprite) {
        let style = { fontSize: '20px', fill: this.gameOptions.cardTextColor, wordWrap: true, wordWrapWidth: sprite.width, align: 'center', backgroundColor: 'transparent' };
    ...
    createFlippingTweens(cardIndex, card, text) {
        ...
        card.events.onInputDown.add(() => { this.onTapCard(cardIndex, card); });
        ...
        card.backflipCardTween.onComplete.add(() => { this.flippingCards[cardIndex] = false; this.computeGameState(); }, this); // once the card has been placed down on the table, we can flip it again
    }
...
class YooFormInputGameComponent {
    ...
    init() {
        this.game = gameFactory(this.name);
